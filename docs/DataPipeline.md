...
```
PressureSensor → 验证(0-20.4kgf/cm^2) → 限幅过滤 → 单位转换 → 结果输出
                 ✓ 在线状态
                 ✓ 合理范围
```
...
```
RegulatingValve → 验证(压力0-20.4kgf/cm^2) → 偏差检测 → 结果输出
                  ✓ 设定值检查
                  ✓ 实际值检查
```
...
- UI 显示与 CSV 导出压力单位统一为 `kgf/cm^2`。
- 设备采集与数据库内部字段可继续保留 `kPa`（兼容历史数据与旧接口）。
- 建议统一换算系数：`1 kgf/cm^2 = 98.0665 kPa`。
