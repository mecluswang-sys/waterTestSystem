        ...
        if (p12Ok && v5StateOk) {
            ...
            if (controlRelay(RELAY_V1_IDX, true)) {
                ...
                setStep(IDX_STEP1, STEP_OK,
                        linkageOk
                            ? QString::fromUtf8("通路正常（PS12 变化 %1 kPa \u2265 阈值 %2 kPa）")
                                  .arg(delta, 0, 'f', 1).arg(linkagePressureThr, 0, 'f', 1)
                            : QString::fromUtf8("通路可能异常（PS12 变化 %1 kPa，阈值 %2 kPa）")
                                  .arg(delta, 0, 'f', 1).arg(linkagePressureThr, 0, 'f', 1));
            } else {
                ...
                if (!skipLeakResult) {
                    ...
                    if (!buildOk) {
                        setStep(IDX_STEP3, STEP_FAIL,
                                QString::fromUtf8("建压不足（PS12=%1 kPa < 最小建压 %2 kPa）")
                                    .arg(p12Build.pressure, 0, 'f', 1).arg(leakBuildMinKpa, 0, 'f', 1));
                        setStep(IDX_STEP4, STEP_FAIL, QString::fromUtf8("建压不足，无法有效判定泄漏"));
                    } else {
                        setStep(IDX_STEP3, RUNNING,
                                QString::fromUtf8("PS12=%1 kPa，保压 %2 ms 中…")
                                    .arg(p12Build.pressure, 0, 'f', 1).arg(leakHoldWaitMs));
                        ...
                        setStep(IDX_STEP3, STEP_OK,
                                QString::fromUtf8("已采集：PS12 %1\u2192%2 kPa（降 %3），PS13 %4\u2192%5 kPa（升 %6）")
                                    .arg(p12Build.pressure, 0, 'f', 1)
                                    .arg(p12Hold.pressure, 0, 'f', 1)
                                    .arg(p12Drop, 0, 'f', 1)
                                    .arg(p13Build.pressure, 0, 'f', 1)
                                    .arg(p13Hold.pressure, 0, 'f', 1)
                                    .arg(p13Rise, 0, 'f', 1));
                        ...
                        setStep(IDX_STEP4, leakOk ? STEP_OK : STEP_FAIL,
                                leakOk
                                    ? QString::fromUtf8("密封正常（PS12压降 %1 kPa \u2264 %2，PS13上升 %3 kPa \u2264 %4）")
                                          .arg(p12Drop, 0, 'f', 1).arg(leakP12DropMaxKpa, 0, 'f', 1)
                                          .arg(p13Rise, 0, 'f', 1).arg(leakP13RiseMaxKpa, 0, 'f', 1)
                                    : QString::fromUtf8("疑似泄漏（PS12压降 %1 kPa 阈值 %2，PS13上升 %3 kPa 阈值 %4）")
                                          .arg(p12Drop, 0, 'f', 1).arg(leakP12DropMaxKpa, 0, 'f', 1)
                                          .arg(p13Rise, 0, 'f', 1).arg(leakP13RiseMaxKpa, 0, 'f', 1));
                    }
                }
